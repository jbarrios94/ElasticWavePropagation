...
def Initial_condition(NumVolx, NumVoly):
...
def CUp(q, c, lim):
    ...
    cmx = q[:,:,1:-1] - q[:,:,:-2]
    cpmx = q[:,:,2:] - q[:,:,:-2]
    qW = q[:,:,1:-1] - dx/2.*  Limiter(cpx, cmx, cpmx, lim, 1)
    qE = q[:,:,1:-1] + dx/2. *  Limiter(cpx, cmx, cpmx, lim, 1)  

    #reconstruction in y
    cpy = q[:,2:,:] - q[:,1:-1,:] 
    cmy = q[:,1:-1,:] - q[:,:-2,:]
    cpmy = q[:,2:,:] - q[:,:-2,:]
    qS = q[:,2:-1,:] - dy/2.*  Limiter(cpy[:,1:,:],cmy[:,1:,:],cpmy[:,1:,:],lim, 2)
    qN = q[:,1:-2,:] + dy/2.*  Limiter(cpy[:,:-1,:],cmy[:,:-1,:],cpmy[:,:-1,:],lim, 2)
    
    
    qNE = qE[:,1:-1,:] + dy/2.*  Limiter(cpy[:,:,1:-1],cmy[:,:,1:-1],cpmy[:,:,1:-1],lim, 2)
    qNW = qW[:,1:-1,1:] + dy/2.*  Limiter(cpy[:,:,2:-1],cmy[:,:,2:-1],cpmy[:,:,2:-1],lim, 2)
    qSE = qE[:,2:-1,:] - dy/2.*  Limiter(cpy[:,1:,1:-1],cmy[:,1:,1:-1],cpmy[:,1:,1:-1],lim, 2)
    qSW = qW[:,2:-1,1:] - dy/2.*  Limiter(cpy[:,1:,2:-1],cmy[:,1:,2:-1],cpmy[:,1:,2:-1],lim, 2)
...
ref_factor = 0
Lim_Infx = 0; Lim_Supx = 3000; NumVolx =  300 * (2**ref_factor) + 1
Lim_Infy = 0; Lim_Supy = 1000; NumVoly =  100 * (2**ref_factor) + 1
t_inicial = 0; t_final = 1000 
CFL = 0.25
...
cp_top = 1.5 
cp_bottom = 5.5 
c = np.zeros((NumVoly + 4, NumVolx + 4))
c[:] = cp_top
cp_i = np.linspace(cp_top, cp_bottom, nlayers)
for i in range(1, nlayers):
        c[i*int(NumVoly // nlayers) + 2:,:] = cp_i[i]
...
CFL = dt * VelMax / dx
q = Initial_condition(NumVolx, NumVoly) 
q = BCx(q, 3, 0)
q = BCx(q, 3, 1)
q = BCy(q, 3, 1)
q = BCy(q, 3, 0)
...
isx = NumVolx//2
isy = np.where(np.linspace(0, 1000, NumVoly) == 20)[0][0]
...
is1 = np.where(np.linspace(Lim_Infx, Lim_Supx, NumVolx) == 1330)[0][0]
gf.plot_seismic_traces([receiver[:, is1]], 0, t_final)
gf.plot_shotrecord(receiver[:, 100*2**ref_factor: 200*2**ref_factor + 1], (0,0), (1000, 1000), 0, t_final, factor=100)
plt.show()
